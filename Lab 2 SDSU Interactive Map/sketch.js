...
function setup() {
  ...
  //finder square, define within the confines of the overview rectangle
  findX = 0; //center
  findY = 0;
  findW = overW;
  findH = overH;
...
function introScreen() {
  ...
  textSize(20);
  textFont('Helvetica');
  text(name + " has " + water_station + " stations.", 605, 320, 200);
}
...
function getFeatureName(grayVal, tbl) {
  ...
  for (var i=1; i<tbl.getRowCount(); i++){
    ...
    if(grayVal == code){
      name = tbl.get(i, "Name");
      water_station = tbl.get(i, "water_stations"); 
    }
  }
}
...
function mousePressed() {
  ...
  //ellipse(pixelX, pixelY, water_station, water_station);
}
...
function mouseDragged() {
  ...
  findX = findX - (xShift / 3); //allows the finder rectangle to move in a similar speed to the mouse when its dragged
  findY = findY - (yShift / 3);
  
  panFromX = panToX;
  panFromY = panToY;
  
  //constraining functions that depend on mouseDragged to keep image within
  originalSize();
  ifLargerImage();
  ifSmallImage();  
}
...
function originalSize(){
  //prevents the image in the original size from being dragged 
...
function ifLargerImage() {
  //defining the parameters for the main image
...
function keyPressed() { 
  ...
  if (key == "r"){
    imgW = rightWall; //return image map to default
    imgH = bottomWall;

    findW = overW; //finder rectangle returns to default 
    findH = overH;
  }
}
...
function mouseWheel(event) {
  ...
  rectangleScaleFactor = event.delta * -(0.001 * (3/4)); //trying to make the rectangle size closer to where image is zoomed in
